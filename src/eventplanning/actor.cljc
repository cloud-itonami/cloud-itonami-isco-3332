(ns eventplanning.actor
  "EventPlanningActor — the ISCO-08 3332 community eventplanning actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one eventplanning operation request
  ...
  The unconditional invariant: the EventPlanningAdvisor can never
  directly commit a record the EventPlanningGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [eventplanning.advisor :as advisor]
            [eventplanning.governor :as governor]
            [eventplanning.store :as store]))

(defn build-graph
  "Build a compiled EventPlanningActor graph. `store` implements
  `eventplanning.store/Store`. `advisor` implements
  `eventplanning.advisor/Advisor` (defaults to `mock-advisor`).
